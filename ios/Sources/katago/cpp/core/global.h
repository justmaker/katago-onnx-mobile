...
#ifdef NDEBUG
// This is so that we can "assert false" in unreachable code branches even in
// the presence of NDEBUG without the compiler complaining about uniniitalized
// values. Ideally sparingly, since the point of NDEBUG presumably is to avoid
// unnecessary runtime checks, but often this is still convenient.
...
// Check if a string consists entirely of digits, and parse the integer,
...
#define STRUCT_NAMED_TRIPLE(A, B, C, D, E, F, G)                               \
  struct G {                                                                   \
    ...
    E F;                                                                       \
    inline G() : B(), D(), F() {}                                              \
    inline G(A s_n_p_arg_0, C s_n_p_arg_1, E s_n_p_arg_2)                      \
        : B(s_n_p_arg_0), D(s_n_p_arg_1), F(s_n_p_arg_2) {}                    \
  }
...
// SHORTCUTS FOR std::map and other
// containers------------------------------------------------

bool contains(const char *str, char c);
bool contains(const std::string &str, char c);

template <typename A> bool contains(const std::vector<A> &vec, const A &elt) {
  for (const A &x : vec)
    if (x == elt)
      return true;
  return false;
...
template <typename A> size_t indexOf(const std::vector<A> &vec, const A &elt) {
...
template <typename A> bool contains(const std::set<A> &set, const A &elt) {
...
B map_get(const std::map<std::string, B> &m, const char *key) {
  typename std::map<std::string, B>::const_iterator it =
      m.find(std::string(key));
  if (it == m.end())
    Global::fatalError(std::string("map_get: key \"") + std::string(key) +
...
namespace Global {
template <typename F> struct CustomScopeGuard {
